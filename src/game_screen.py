...
from config import WHITE, INSPER_RED, FPS, STILL, LEFT, RIGHT, window_width, window, clock
    ...
    while state != QUIT:
        ...
        if state == RUNNING:
            hits = pygame.sprite.groupcollide(
                all_enemies, all_bullets, True, True, pygame.sprite.collide_mask)
            for enemy in hits:
            ...
            hits = pygame.sprite.spritecollide(
                player, all_enemies, True, pygame.sprite.collide_mask)
            if len(hits) > 0:
                ...
                explosion_duration = explosion.frame_ticks * \
                    len(explosion.explosion_anim) + 50

            hits = pygame.sprite.spritecollide(
                player, all_platforms, False, pygame.sprite.collide_mask)
            if hits:
                player.speedy = 0
        ...
        text_surface = assets['score_font'].render(
            '{:08d}'.format(score), True, WHITE)
        text_rect = text_surface.get_rect()
        text_rect.midtop = (window_width / 2, 10)
        window.blit(text_surface, text_rect)

        text_surface = assets['score_font'].render(
            chr(9829) * lives, True, INSPER_RED)
        text_rect = text_surface.get_rect()
